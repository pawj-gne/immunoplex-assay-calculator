---
name: commit
description: Use when about to run `git commit`, when the user asks to "commit", "commit changes", "commit this", or when staging changes for a commit. Enforces Conventional Commits prefix, the dev-branch rule (never commit to main), conversation-scoped atomic commits, and the always-push-after-commit rule.
user-invocable: true
---
