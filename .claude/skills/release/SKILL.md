---
name: release
description: Use when the user asks for a "release", "cut a release", "ship a version", "bump the version", "tag", "create a PR for release", or when preparing to merge a dev branch into main. Runs pre-flight cleanup, full build verification (tsc, npm build, Windows exe), semver version bump with user approval, PR creation to the current dev branch, and post-merge tagging/cleanup.
user-invocable: true
---
